...
example : Group ℤ where
  ...
  mul_inv_left := by
    exact fun a ↦ Int.add_left_neg a
...
example : Group Aˣ where
  ...
  inv := fun a ↦ a⁻¹
  mul_assoc := by
    exact fun a b c ↦ mul_assoc a b c
  one_mul := by
...
def Subgroup.centralizer : Subgroup G where
  ...
  mul_mem' := by
    ...
    -/
    intro _ _ ha hb _ hx
    rw [mul_assoc, hb _ hx, ← mul_assoc, ha _ hx, mul_assoc]
  inv_mem' := by
    intro a ha x hx
    apply mul_left_cancel a
    simp [← mul_assoc, ha _ hx]
...
def Subgroup.normalizer : Subgroup G where
  ...
  inv_mem' := by
    intro a ha x
    have := ha (a⁻¹ * x * a) -- `x`として`a⁻¹ * x * a`を適応すると、これは本質的にゴールと同じ
    simp [this, ← mul_assoc]
